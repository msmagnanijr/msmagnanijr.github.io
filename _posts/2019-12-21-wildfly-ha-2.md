...
In [Chapter I](http://mlab.run/2019/12/18/wildfly-ha-1), we set up the high availability of the Web layer. In this chapter we will install WildFly and configure the Master/Slave topology.
...
/usr/local/wildfly/wildfly-18.0.1.Final
[root@server-slave-1 ~]# cp -Rap domain slave1
{% endhighlight %}

Edit the host-slave.xml file. Add slave name and update the existing servers. The servers must be called "server-marketing-1" and "server-accounting-1".  Servers must belong to their own group: Marketing and Accounting. 
...
<?xml version='1.0' encoding='UTF-8'?>
##around line 3
<host xmlns="urn:jboss:domain:10.0" name="slave1">
    <extensions>
##suppressed
{% endhighlight %}
...
if [ "x$JAVA_OPTS" = "x" ]; then
   ...
   ##Add this line
   JAVA_OPTS="$JAVA_OPTS -Djboss.domain.base.dir=/usr/local/wildfly/wildfly-18.0.1.Final/slave1 -Djboss.host.default.config=host-slave.xml -Djboss.domain.master.address=10.0.0.68 -Djboss.bind.address=10.0.0.66"
else
##suppressed
{% endhighlight %}
...
{% highlight bash %}
[root@server-domain ~]# tailf /usr/local/wildfly/wildfly-18.0.1.Final/master/log/
2019-12-21 08:52:54,725 INFO  [org.jboss.as.domain.controller] (Host Controller Service Threads - 34) WFLYHC0019: Registered remote slave host "slave1", JBoss WildFly Full 18.0.1.Final (WildFly 10.0.3.Final)
{% endhighlight %}

Open the management console again to see the updated topology: http://10.0.0.68:9990/console

![](/images/201912-hawildfly-11.png)

It is! Now we have our topology with One Host Master, Two Host Slaves and Four WildFly Instances.
