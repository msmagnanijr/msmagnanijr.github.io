...
Now edit the domain.xml file and update the existing groups. Now the groups must be called "marketing" and "accounting" that will use the "full-ha" profile and "full-ha-sockets".
...
Restart the WildFly service. Note that now a new topology will be presented:
