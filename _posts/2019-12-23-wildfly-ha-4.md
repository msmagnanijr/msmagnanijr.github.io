...
##  WildFly Instance Scalability with Ansible

We will scale our instances manually using JBoss CLI and later this will be done automatically using a trigger via AlertManager and Ansible AWX.

So the first step is to test the creation of a new instance using JBoss CLI.

Use the master host itself for these tests. Create a file called create-servers.cli:
...
Note that a few seconds later the instance is already available on the balancer and also the deployment will be deployed automatically. This gives us a very interesting scalability in our WildFly environment.
